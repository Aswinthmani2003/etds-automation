...
def parse_excel(excel_file):
    ...
    for sheet_name in wb.sheetnames:
        ...
        # Detect token and target columns
        col_stats = {}
        for row in rows:
        ...
        token_col = max((k for k, v in col_stats.items() if v['t20'] > 0),
                       default=-1, key=lambda k: col_stats[k]['t20'])
        target_col = max((k for k, v in col_stats.items() if k != token_col and v['t15'] > 0),
                        default=-1, key=lambda k: col_stats[k]['t15'])
    ...
    return token_map, dup_tokens, rows_scanned
    ...
    try:
        ...
        for page_num, image in enumerate(images, 1):
            ...
            for scale in [1, 0.75, 0.5]:
                if scale != 1:
                    w, h = image.size
                    image_scaled = image.resize((int(w * scale), int(h * scale)))
                else:
                    image_scaled = image

                # Decode barcode
                decoded = decode(image_scaled)
                if decoded:
                    for obj in decoded:
...
def process():
    ...
    try:
        # Parse Excel
        token_map, dup_tokens, rows_scanned = parse_excel(excel_file)
        ...
        for pdf_file in pdf_files:
            ...
            if not token:
                ...
                if not hit:
                    row['status'] = 'unmatched'
                    row['reason'] = 'Token not found in Excel'
                    unmatched += 1
                else:
                    ...
                    matched += 1
                    if page and page > 1:
                        row['reason'] = 'Barcode found on page ' + str(page)

            results.append(row)

        excel_note = str(len(token_map)) + ' token rows loaded from Excel'
        if dup_tokens:
            excel_note += ' | ' + str(len(dup_tokens)) + ' duplicate token(s)'
            ...
            for i, pdf_file in enumerate(pdf_files):
                if i < len(results):
                    ...
                    if result['status'] == 'matched':
                        zf.writestr(result['newName'], pdf_file.read())
                    else:
                        zf.writestr('unmatched/' + result['original'], pdf_file.read())

        zip_buffer.seek(0)
        return send_file(zip_buffer, mimetype='application/zip', as_attachment=True, download_name='renamed_etds.zip')
    except Exception as e:
        return jsonify({'error': str(e)}), 500
